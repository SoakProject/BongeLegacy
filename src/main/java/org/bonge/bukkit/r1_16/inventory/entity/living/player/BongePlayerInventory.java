package org.bonge.bukkit.r1_16.inventory.entity.living.player;
...
public class BongePlayerInventory extends BongeInventory<org.spongepowered.api.item.inventory.entity.PlayerInventory> implements CommonPlayerInventory {
    ...
    private @Nullable ItemStack getEquipment(Supplier<? extends EquipmentType> type){
        Optional<org.spongepowered.api.item.inventory.ItemStack> opItem = this.spongeValue.equipment().peek(type);
        if(!opItem.isPresent()){
            return null;
        }
    ...
    public void setItem(@NotNull EquipmentSlot slot, @Nullable ItemStack item) {
        throw new NotImplementedException("setItem(EquipmentSlot, ItemStack)");
    }

    @NotNull
    @Override
    public ItemStack getItem(@NotNull EquipmentSlot slot) {
        throw new NotImplementedException("setItem(EquipmentSlot)");
    }
    ...
    private void setEquipment(Supplier<? extends EquipmentType> type, ItemStack stack){
        if(stack == null) {
            this.spongeValue.equipment().set(type, org.spongepowered.api.item.inventory.ItemStack.empty());
            return;
        }
        ...
        this.spongeValue.equipment().set(type, stack1);
    }
    ...
    public int getHeldItemSlot() {
        return this.spongeValue.hotbar().selectedSlotIndex();
    }

    @Override
    public void setHeldItemSlot(int slot) {
        this.spongeValue.hotbar().setSelectedSlotIndex(slot);
    }

    @Override
    public @Nullable BongePlayer getHolder() {
        Optional<Player> opPlayer = this.spongeValue.carrier();
