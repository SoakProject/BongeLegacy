package org.bonge.bukkit.r1_14.inventory;
...
public class BongeInventoryView extends InventoryView {
    ...
    public ItemStack getCursor0(){
        Optional<org.spongepowered.api.item.inventory.ItemStack> opStack = this.topContainer.getCursor();
        if(opStack.isPresent()){
            return new ItemStack(opStack.get());
        }
        return null;
    }
    ...
    public @NotNull String getTitle() {
        ...
        return Bonge.getInstance().convert(this.top.getSpongeValue().get(Keys.DISPLAY_NAME.get()).get());
    }
}
